...
@export var deltaGouttesMin = 0.2
@export var deltaGouttesMax = 0.8
var min_initial
var max_initial
...
func _ready():
	...
	timer.connect("timeout", Callable(self, "goutte"))
	timer.start()
	r.randomize()
...
func goutte ():
	...
	var newGoutte = pluies[r.randi_range(0,pluies.size()-1)].instantiate()
	add_child(newGoutte)
